...
def generate_source_reconstruction() -> None:
    ...
    reconstructed = source["reconstructed_leads"].to_list()
...
def generate_cpl_error_reduction() -> None:
    ...
    """
    validation = pl.read_csv(VALIDATION / "paid_kpi_validation.csv").sort("source_canonical")

    labels = validation["source_canonical"].to_list()

    platform_error = (validation["platform_cpl_error_rate"].abs() * 100).to_list()

    reconstructed_error = (validation["reconstructed_cpl_error_rate"].abs() * 100).to_list()
    ...
    ax.set_title("CPL error relative to hidden truth")

    ax.set_ylabel("Absolute CPL error (%)")
...
def generate_acquisition_composition() -> None:
    ...
    """
    sources = pl.read_csv(REPORT / "reconstructed_sources.csv").sort(
        "reconstructed_leads",
        descending=True,
    )

    labels = sources["source_canonical"].to_list()

    confirmed = sources["cross_system_confirmed"].to_list()

    crm_only = sources["crm_only_entities"].to_list()

    marketing_only = sources["marketing_only_entities"].to_list()
    ...
    ax.set_title("Reconstructed acquisition universe")
...
def generate_client_cpl_comparison() -> None:
    ...
    """
    paid = pl.read_csv(REPORT / "reconstructed_paid_kpis.csv").sort("source_canonical")

    labels = paid["source_canonical"].to_list()

    series = {
        "Platform reported": (paid["platform_reported_cpl"].to_list()),
        "CRM-confirmed": (paid["crm_confirmed_cpl"].to_list()),
        "Reconstructed": (paid["reconstructed_cpl"].to_list()),
    }
...
def main() -> None:
    ...
    print(f"Wrote 5 CRM portfolio figures to {FIG}")

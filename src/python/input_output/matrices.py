...
def build_technical_matrix(
    ...
    if list(D.columns) != list(Bn.index):
        raise ValueError("Product codes are not aligned between D and Bn.")
...
def build_leontief_inverse(
    ...
    if A.shape[0] != A.shape[1]:
        raise ValueError("Technical coefficient matrix must be square.")
    ...
    values = np.linalg.inv(identity - A.to_numpy())

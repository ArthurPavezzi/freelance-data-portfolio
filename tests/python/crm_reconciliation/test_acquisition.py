...
def test_reconstructed_universe_uses_entities_not_records() -> None:
    universe = build_reconstructed_acquisition_universe(
        _triaged(),
        _deduped(),
    )
    ...
    assert universe.filter(pl.col("acquisition_entity_id") == "WCR1").height == 1
...
def test_unknown_source_crm_entity_is_excluded() -> None:
    ...
    assert "WCR2" not in universe["acquisition_entity_id"].to_list()
...
def test_source_summary_reconstructs_lead_composition() -> None:
    ...
    summary = build_acquisition_source_summary(universe)

    google = summary.filter(pl.col("source_canonical") == "Google Ads")

    assert google["reconstructed_leads"][0] == 3

    assert google["cross_system_confirmed"][0] == 1

    assert google["crm_only_entities"][0] == 1

    assert google["marketing_only_entities"][0] == 1
...
def test_reconstructed_cpl_uses_entity_count() -> None:
    ...
    kpis = build_reconstructed_paid_kpis(
        universe,
        spend,
    )

    google = kpis.filter(pl.col("source_canonical") == "Google Ads")

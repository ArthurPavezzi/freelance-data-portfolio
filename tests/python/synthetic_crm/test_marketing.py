...
def test_lead_generation() -> None:
    ...
    assert (
        leads["created_at"].min().date().isoformat()
        >= config.start_date
    )

    assert (
        leads["created_at"].max().date().isoformat()
        <= config.end_date
    )
...
def test_lead_timestamps_have_minute_precision() -> None:
    ...
    assert (
        leads
        .filter(
            pl.col("created_at").dt.microsecond() != 0
        )
        .height
        == 0
    )
